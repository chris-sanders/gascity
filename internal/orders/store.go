...
// ExternalDeliveryIdempotencyKey returns the stable native-root key for one
// externally correlated order dispatch. Hashing keeps provider-shaped delivery
// identifiers bounded and safe as opaque bead metadata while preserving
// equality across controller restarts.
func ExternalDeliveryIdempotencyKey(scoped, deliveryID string) string {
	input := strings.TrimSpace(scoped) + "\x00" + strings.TrimSpace(deliveryID)
	sum := sha256.Sum256([]byte(input))
	return "order-external:" + hex.EncodeToString(sum[:])
}
